import React from 'react';
import styled from 'styled-components';
import {useFilterContext} from '../context/filter_context';
import {getUniqueValues, formatPrice} from '../utils/helpers';
import {FaCheck} from 'react-icons/fa';

const Filters = () => {
  const {
    filter: {
      query,
      category,
      company,
      colors,
      min_price,
      max_price,
      price,
      shipping,
    },
    updateFilter,
    clearFilter,
    all_products: products,
  } = useFilterContext();

  const selectedCategory = getUniqueValues(products, 'category');
  const selectedCompany = getUniqueValues(products, 'company');
  const selectedColor = getUniqueValues(products, 'colors');

  return (
    <Wrapper>
      <div className="content">
        <form onSubmit={(e) => e.preventDefault()}>
          <div className="form-contorl">
            <input
              onChange={updateFilter}
              className="search-input"
              type="text "
              name="query"
              placeholder="search "
              value={query}
            />
          </div>
          <div className="form-contorl">
            <h5>category</h5>
            <div>
              {selectedCategory.length > 1 &&
                selectedCategory.map((item, index) => {
                  return (
                    <button
                      name="category"
                      className={`${
                        category === item.toLowerCase() ? 'active' : null
                      }`}
                      key={index}
                      value={item}
                      onClick={updateFilter}
                    >
                      {item}
                    </button>
                  );
                })}
            </div>
          </div>
          <div className="form-contorl">
            <h5>company</h5>
            <select
              value={company}
              name="company"
              className="company"
              onChange={updateFilter}
            >
              {selectedCompany.map((item, index) => {
                return (
                  <option key={index} value={item}>
                    {item}
                  </option>
                );
              })}
            </select>
          </div>
          <div className="form-contorl">
            <h5>colors</h5>
            <div className="colors">
              {selectedColor.length > 1 &&
                selectedColor.map((item, index) => {
                  if (item === 'all') {
                    return (
                      <button
                        name="colors"
                        className={`${
                          colors === 'all' ? 'all-btn active' : 'all-btn'
                        }`}
                        key={index}
                        value="all"
                        onClick={updateFilter}
                      >
                        all
                      </button>
                    );
                  }
                  return (
                    <button
                      name="colors"
                      className={`${
                        colors === item ? 'color-btn active' : 'color-btn'
                      }`}
                      key={index}
                      value={item}
                      onClick={updateFilter}
                      style={{backgroundColor: item}}
                    >
                      {colors === item ? <FaCheck /> : null}
                    </button>
                  );
                })}
            </div>
          </div>
          <div className="form-contorl">
            <h5>price</h5>
            <p className="price">{formatPrice(price)}</p>
            <input
              type="range"
              name="price"
              onChange={updateFilter}
              min={min_price}
              max={max_price}
              value={price}
            />
          </div>
          <div className="form-control shipping">
            <label htmlFor="shipping">free shipping</label>
            <input
              type="checkbox"
              name="shipping"
              id="shipping"
              onChange={updateFilter}
              checked={shipping}
            />
          </div>

          <button className="clear-btn" onClick={clearFilter}>
            clear filters
          </button>
        </form>
      </div>
    </Wrapper>
  );
};

const Wrapper = styled.section`
  .form-control {
    margin-bottom: 1.25rem;
    h5 {
      margin-bottom: 0.5rem;
    }
  }
  .search-input {
    padding: 0.5rem;
    background: var(--clr-grey-10);
    border-radius: var(--radius);
    border-color: transparent;
    letter-spacing: var(--spacing);
  }
  .search-input::placeholder {
    text-transform: capitalize;
  }

  button {
    display: block;
    margin: 0.25em 0;
    padding: 0.25rem 0;
    text-transform: capitalize;
    background: transparent;
    border: none;
    border-bottom: 1px solid transparent;
    letter-spacing: var(--spacing);
    color: var(--clr-grey-5);
    cursor: pointer;
  }
  .active {
    border-color: var(--clr-grey-5);
  }
  .company {
    background: var(--clr-grey-10);
    border-radius: var(--radius);
    border-color: transparent;
    padding: 0.25rem;
  }
  .colors {
    display: flex;
    align-items: center;
  }
  .color-btn {
    display: inline-block;
    width: 1rem;
    height: 1rem;
    border-radius: 50%;
    background: #222;
    margin-right: 0.5rem;
    border: none;
    cursor: pointer;
    opacity: 0.5;
    display: flex;
    align-items: center;
    justify-content: center;
    svg {
      font-size: 0.5rem;
      color: var(--clr-white);
    }
  }
  .all-btn {
    display: flex;
    align-items: center;
    justify-content: center;
    margin-right: 0.5rem;
    opacity: 0.5;
  }
  .active {
    opacity: 1;
  }
  .all-btn .active {
    text-decoration: underline;
  }
  .price {
    margin-bottom: 0.25rem;
  }
  .shipping {
    display: grid;
    grid-template-columns: auto 1fr;
    align-items: center;
    text-transform: capitalize;
    column-gap: 0.5rem;
    font-size: 1rem;
  }
  .clear-btn {
    background: var(--clr-red-dark);
    color: var(--clr-white);
    padding: 0.25rem 0.5rem;
    border-radius: var(--radius);
  }
  @media (min-width: 768px) {
    .content {
      position: sticky;
      top: 1rem;
    }
  }
`;

export default Filters;
